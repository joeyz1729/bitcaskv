...
func TestSkipList1_Put(t *testing.T) {
	skl := NewSkipList1()
...
func TestSkipList1_Get(t *testing.T) {
	skl := NewSkipList1()
...
func TestSkipList1_Delete(t *testing.T) {
	skl := NewSkipList1()
	res1 := skl.Put(nil, &data.LogRecordPos{Fid: 1, Offset: 100})
	assert.Nil(t, res1)
...
func TestSkipList1_Iterator(t *testing.T) {
	skl := NewSkipList1()
	// 1.SkipList 为空的情况
	...
	assert.Equal(t, iter2.Key(), []byte("ccde"))
	assert.Equal(t, iter2.Value(), &data.LogRecordPos{Fid: 1, Offset: 10})
	iter2.Next()
